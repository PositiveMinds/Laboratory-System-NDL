...
import type { OrderDetail as OD, Payment, ReceiptData, ResultsReportData, TestItem } from '../types';
...
export default function OrderDetailPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [o, pays] = await Promise.all([getOrder(orderId), getOrderPayments(orderId)]);
      setOrder(o);
      setPayments(pays);
    } finally {
      setLoading(false);
  ...
  const handleStatusChange = async (newStatus: string) => {
    const result = await Swal.fire({ title: 'Update Status', text: `Change order status to "${newStatus}"?`, icon: 'question', showCancelButton: true, confirmButtonColor: '#f54927' });
  ...
  const handleAddPayment = async () => {
    ...
    if (!amount || amount <= 0) {
      Swal.fire({ icon: 'warning', title: 'Invalid Amount', text: 'Enter a valid payment amount.', confirmButtonColor: '#f54927' });
      return;
    }
    setPaying(true);
    try {
  ...
  const handleVerify = async () => {
    const result = await Swal.fire({ title: 'Verify Results?', text: 'This marks you as the verifying scientist for this order.', icon: 'question', showCancelButton: true, confirmButtonColor: '#f54927' });
    if (!result.isConfirmed) return;
    try {
  ...
  const handlePrintResults = async () => {
    try {
      const data = await getResultsReport(orderId);
  ...
  const handleDiscount = async () => {
    ...
    const { value: input } = await Swal.fire({
      ...
      preConfirm: () => {
        ...
        if (amt > order.total_amount) { Swal.showValidationMessage('Discount cannot exceed total'); return false; }
        return { amt, reason };
      },
    });
    if (!input) return;
  ...
  return (
    <div>
      {receipt && <div className="print-container" ref={printRef}><PrintReceipt data={receipt} /></div>}
      {reportData && <div className="print-container"><PrintResults data={reportData} /></div>}
      {showLabel && order && (
        <div className="print-container">
          <PrintLabel orderNumber={order.order_number} patientName={order.patient_name} patientRef={order.patient_ref} orderDate={order.order_date} copies={2} />
        </div>
      )}
      ...
      <div className="no-print">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 20, alignItems: 'start' }}>
            ...
            <div className="card" style={{ marginBottom: 16 }}>
              ...
              {Object.entries(grouped).map(([cat, items]) => (
                <div key={cat} style={{ marginBottom: 12 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', color: 'var(--primary-color)', marginBottom: 6, letterSpacing: 0.5 }}>{cat}</div>
                    ...
                    <tbody>
                      {payments.map(pay => (
                        <tr key={pay.id}>
                          ...
                          <td className="text-right font-bold" style={{ color: '#10b981' }}>{fmtUGX(pay.amount)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          <div className="order-summary-panel">
            ...
            <div className="summary-item"><span>Total Amount</span><span className="price">{fmtUGX(order.total_amount)}</span></div>
            {order.discount_amount > 0 && (
              <div className="summary-item"><span style={{ color: '#f59e0b' }}>Discount</span><span style={{ color: '#f59e0b', fontWeight: 600 }}>- {fmtUGX(order.discount_amount)}</span></div>
            )}
            <div className="summary-item"><span>Amount Paid</span><span style={{ color: '#10b981', fontWeight: 600 }}>{fmtUGX(order.amount_paid)}</span></div>
            <div className="summary-total" style={{ color: order.balance > 0 ? 'var(--danger-color)' : '#10b981' }}>
              <span>Balance</span><span>{fmtUGX(order.balance)}</span>
            </div>
